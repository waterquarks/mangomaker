...
function watchHighestOI(mkt, n) {
    return __awaiter(this, void 0, void 0, function* () {
        const cfg = (0, config_1.getPerpMarketByBaseSymbol)(groupIds, mkt);
        console.log('getMangoGroup');
        const group = yield client.getMangoGroup(mangoGroupKey);
        console.log('getAllMangoAccounts');
        const mangoAccounts = yield client.getAllMangoAccounts(group);
...
function watchHighestSpotBalance(mkt, n) {
    return __awaiter(this, void 0, void 0, function* () {
        const cfg = (0, config_1.getSpotMarketByBaseSymbol)(groupIds, mkt);
        console.log('getMangoGroup');
        const group = yield client.getMangoGroup(mangoGroupKey);
        console.log('getAllMangoAccounts');
        const mangoAccounts = yield client.getAllMangoAccounts(group);
        ...
        const cache = yield group.loadCache(connection);
        mangoAccounts.sort((a, b) => {
            const aBalance = a.getUiDeposit(cache.rootBankCache[cfg.marketIndex], group, cfg.marketIndex);
            const bBalance = b.getUiDeposit(cache.rootBankCache[cfg.marketIndex], group, cfg.marketIndex);
