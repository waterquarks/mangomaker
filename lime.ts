...
Makes Serum markets supported by Mango.

Example command:
KEYPAIR_PATH=~/.config/solana/id.json MANGO_GROUP=devnet.2 MANGO_ACCOUNT=BFLAGijqDyRnK93scRizT8YSotrvdxhdJyWYZch6yMMW MARKET=MNGO yarn ts-node src/scripts/lime.ts
...
const main = async () => {
    ...
    const token = getTokenBySymbol(mangoGroupConfig, MARKET!)
    ...
    const quote = async () => {
        ...
        const instructions = await Promise.all([
            ...
            createSpotOrder2Instruction(mangoClient, mangoGroup, mangoAccount, spotMarket, payer, 'buy', bidPrice, 10, 'postOnly', undefined, true),
            createSpotOrder2Instruction(mangoClient, mangoGroup, mangoAccount, spotMarket, payer, 'sell', askPrice, 10, 'postOnly', undefined, true)
        ])
        ...
        const meta = `${tokenPrice} ${[bidPrice.toString(), size.toString()]} ${[askPrice.toString(), size.toString()]}`
